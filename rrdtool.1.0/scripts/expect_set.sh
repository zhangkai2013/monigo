...
expect {
     "*yes/no*" {
        ...
        #interact
	send -- "sed -i \"425a\exec .1.3.6.1.4.1.2021.63 tcp  /bin/sh  /opt/mrtg/tcp.sh\"  /etc/snmp/snmpd.conf"
	expect -exact "sed -i \"425a\exec .1.3.6.1.4.1.2021.63 tcp  /bin/sh  /opt/mrtg/tcp.sh\"  /etc/snmp/snmpd.conf"
	send -- "\r"
        expect "*#"
	#send -- "exit\r"
        exit 0
